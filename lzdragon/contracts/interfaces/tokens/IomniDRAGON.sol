...
interface IomniDRAGON {
    ...
    /**
     * @dev Get operational funds balance (wrapped native tokens) of the contract
     * @return Operational funds balance
     */
    function getOperationalFundsBalance() external view returns (uint256);
    ...
    /**
     * @dev DEPRECATED: Fee collection is now handled atomically within token transfers
     * Buy fees are now deducted from token amounts during transfers from DEX pairs
     * This ensures fees cannot be bypassed by direct pair interaction
     */
