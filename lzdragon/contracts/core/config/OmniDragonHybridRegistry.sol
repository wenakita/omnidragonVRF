...
contract OmniDragonHybridRegistry is IOmniDragonHybridRegistry, Ownable {
    ...
    /**
     * @notice Register a new chain with wrapped native symbol support
     */
    function registerChain(
        ...
        // Auto-determine symbol if not provided
        string memory symbol = bytes(_wrappedNativeSymbol).length > 0 ? 
            _wrappedNativeSymbol : 
            _getDefaultWrappedNativeSymbol(_chainId);
        ...
        // Prevent DoS by limiting supported chains
    ...
    /**
     * @notice Update chain configuration with wrapped native symbol support
     */
    function updateChain(
        ...
        // Update symbol (auto-determine if not provided)
        if (bytes(_wrappedNativeSymbol).length > 0) {
            config.wrappedNativeSymbol = _wrappedNativeSymbol;
        } else {
            config.wrappedNativeSymbol = _getDefaultWrappedNativeSymbol(_chainId);
        }
    ...
    function getWrappedNativeSymbol(uint16 _chainId) external view override returns (string memory) {
        ...
        // Return stored symbol if available, otherwise determine from chain ID
        if (bytes(config.wrappedNativeSymbol).length > 0) {
            return config.wrappedNativeSymbol;
        }
        
        // Fallback to default symbol determination
        return _getDefaultWrappedNativeSymbol(_chainId);
    }

    /**
     * @notice Get both wrapped native token address and symbol for a chain
     */
    function getWrappedNativeInfo(uint16 _chainId) external view override returns (address tokenAddress, string memory symbol) {
        ChainConfig memory config = chainConfigs[_chainId];
        tokenAddress = config.wrappedNativeToken;
        
        // Return stored symbol if available, otherwise determine from chain ID
        if (bytes(config.wrappedNativeSymbol).length > 0) {
            symbol = config.wrappedNativeSymbol;
        } else {
            symbol = _getDefaultWrappedNativeSymbol(_chainId);
        }
    }
