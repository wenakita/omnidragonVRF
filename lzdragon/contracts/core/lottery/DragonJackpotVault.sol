...
// Interface for WNATIVE token (WETH/WSONIC)
interface IWNATIVE {
...
contract DragonJackpotVault is IDragonJackpotVault, Ownable, ReentrancyGuard {
  ...
  /**
   * @dev Allow receiving ETH and automatically wrap it to WNATIVE
   * @dev SECURITY FIX: Automatically wrap ETH to prevent accounting discrepancy
   * Critical vulnerability fix - CVE-2024-AUDIT-011
   */
  receive() external payable nonReentrant {
    require(wrappedNativeToken != address(0), 'Wrapped token not set for ETH');
    require(msg.value > 0, 'No ETH sent');

    // SECURITY FIX: Automatically wrap the received ETH into WNATIVE
    IWNATIVE(wrappedNativeToken).deposit{value: msg.value}();
    
    // Track wrapped tokens in accounting
  ...
  function enterJackpotWithDragon(address user, uint256 amount) external override {
    ...
    // This function can be implemented to handle Dragon token entries
    // For now, we'll emit an event and track the entry
  ...
  function enterJackpotWithWrappedNativeToken(address user, uint256 amount) external override {
    ...
    // Transfer wrapped native tokens from the caller to this contract
